...
nexusipam/core/ed25519.py, dnsmaqmgr/core/ed25519.py, and here). That is the
whole point of it existing: `cryptography` is not a dependency of any of those
apps, and adding one would mean a `fleet-deploy.sh --deps` pass across twelve
nodes — the PyYAML/AI-node crash-loop precedent. Verification needs no library.

In this repo it matters twice over: pyproject promises the CLI has no runtime
dependencies at all, and the web extra is three packages. Neither budget moves
to gain SSO.
